...
class Processor(ABC):
    def __init__(self, tokenizer: Union[PreTrainedTokenizer, str], max_seq_len: int, label_list: List[str], **kwargs):
        if isinstance(tokenizer, PreTrainedTokenizer):
            self.tokenizer = tokenizer
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer, **kwargs, use_fast=False)
...
class TextClassificationProcessor(Processor):
    ...
    def __init__(self, tokenizer: Union[PreTrainedTokenizer, str], max_seq_len: int, label_list: List[str],
                 multilabel: bool = False, quotechar: str = '"', skiprows: int = 1, **kwargs, ):
        super().__init__(tokenizer=tokenizer, max_seq_len=max_seq_len, label_list=label_list, **kwargs)
    ...
    def get_examples(self, training_data: Dict) -> List[TextClassificationExample]:
        ...
        """
        assert len(training_data["text"]) == len(
            training_data["labels"]), f"{len(training_data['text'])} text and {len(training_data['labels'])} labels"
        examples = []
        is_contain_position = "positions" in training_data
        for ii in tqdm(range(len(training_data["text"]))):
            label = training_data["labels"][ii]
            assert label in self.label_list, f"Non exist label: '{label}' in label list: {self.label_list}."
            ...
            examples.append(TextClassificationExample(uid=f"{ii}",
                                                      doc_tokens=doc_tokens,
                                                      label=label,
                                                      positions=positions))
        return examples

    def _convert_example_to_feature(self, example: TextClassificationExample,
                                    label_to_idx: Dict[str, int]) -> TextClassificationFeatures:
        all_positions = []
        all_doc_tokens = []
        ...
        encoded_dict = self.tokenizer.encode_plus(all_doc_tokens, padding="max_length", truncation=True,
                                                  max_length=self.max_seq_len, return_token_type_ids=True)
    ...
    def convert_examples_to_features(self,
                                     examples: List[TextClassificationExample]) -> List[TextClassificationFeatures]:
        ...
        for ii in tqdm(range(len(examples))):
            features.append(self._convert_example_to_feature(examples[ii], label_to_idx))
        return features

    def features_to_dataset(self, features: List[TextClassificationFeatures],
                            mode: Union[str, Mode] = Mode.train) -> TensorDataset:
        """Get Pytorch Dataset object from list of classification features"""
        if isinstance(mode, Mode):
            mode = mode.value
        dataset = [torch.tensor([f.input_ids for f in features], dtype=torch.long),
...
class BERTParetoMoEProgram(ABC):
    def __init__(self, cfg: DictConfig, __file__: str):
        ...
        self.result_dir = RESULTS_DIR / os.path.basename(__file__).split(".")[0] / f"version_{cfg.version}"

        # Setup fabric.
        self.fabric = Fabric(accelerator="cuda",
                             loggers=TensorBoardLogger(save_dir=self.result_dir, name="tb_logs", version=""),
    ...
    def load_datasets(self):
        ...
        assert cfg.batch_size % cfg.num_devices == 0, "Batch size must be divisible by num_devices"
        ...
        dataset_mapping = {"task1": load_imdb_dataset(Mode.train), "task2": load_imdb_dataset(Mode.train)}

        def get_dataloader(data: Dict[str, Any], labels: List[str]) -> DataLoader:
            dataprocessor = TextClassificationProcessor(tokenizer=tokenizer,
                                                        max_seq_len=512, label_list=labels)
            ...
            dataloader = DataLoader(dataset, batch_size=cfg.batch_size, shuffle=False if cfg.num_devices > 1 else True,
                                    sampler=(
                                        DistributedSampler(dataset, shuffle=True) if cfg.num_devices > 1 else None))
            return dataloader

        self.train_loaders = {task_name: get_dataloader(*data) for task_name, data in dataset_mapping.items()}
        ...
        self.task_num_labels_mapping = {task_name: len(data[1]) for task_name, data in dataset_mapping.items()}
        self.train_loader_iters = [iter(itertools.cycle(dd)) for dd in self.train_loaders.values()]
    ...
    def load_model(self):
        ...
        log.info("Loading pretrained model. The classification layer can be different between tasks but in this simple "
        ...
        self.finetuned_backbone: Dict[str, nn.Module] = {task: cast(BertLayer, model.bert) for task, model in
                                                         self.finetuned_models.items()}
        ...
        with timeit_context("building model"):
            if self.cfg.partial:
                ...
                # model merging.
                model: nn.Module = task_arithmetic_merge_modules(pretrained_backbone,
                ...
                for layer_idx in range(self.pretrained_model.config.num_hidden_layers):
                    cast(BertLayer, model.encoder.layer[layer_idx]).output.dense = (
                        ParetoWeightEnsemblingModule(
                            base_model=model.encoder.layer[layer_idx].output.dense,
                            expert_models=[self.finetuned_backbone[task].encoder.layer[layer_idx].output.dense
                                           for task in self.finetuned_backbone.keys()],
                            init_lambda=cfg.init_lambda, fix_base_model_and_experts=True,
                            router_hidden_layers=cfg.router_hidden_layers))
            else:
                raise NotImplementedError("Full model weight ensembling not implemented")
    ...
    def compute_loss(self, model: nn.Module, ray: Tensor, losses: List[Tensor]):
        ...
        loss = epo_solver.get_weighted_loss(losses, ray, tuple(filter(lambda p: p.requires_grad, model.parameters())))
        return loss
    ...
    def train(self):
        ...
        print(self.model)
        classifiers = {task: cast(BertForSequenceClassification, m).classifier.requires_grad_(False).to(device)
        ...
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, backbone.parameters()), lr=cfg.lr)

        backbone, optimizer = self.fabric.setup(backbone, optimizer)
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=cfg.num_steps,
                                                                  eta_min=cfg.lr * 0.1)
        ...
        backbone.train()
        for step_idx in tqdm(range(1, 1 + cfg.num_steps), "training", dynamic_ncols=True):
            # Sample a preference ray.
            ray = torch.from_numpy(
                np.random.dirichlet((cfg.alpha,) * num_objectives, 1).astype(np.float32).flatten()).to(device)
            ...
            for dataset_idx, dataset_name in enumerate(cfg.tasks):
                ...
                forward_model.num_labels = self.finetuned_models[dataset_name].num_labels

                outputs = torch.func.functional_call(
                    forward_model,
                    parameter_and_buffer_dicts={"classifier." + k: v for k, v in
                                                classifiers[dataset_name].state_dict(keep_vars=True).items()},
                    args=tuple(),
                    kwargs=dict(input_ids=batch[0].to(device),
...
if __name__ == "__main__":
    config = DictConfig({"model": "bert-base-uncased", "version": None, "num_devices": 1,
                         "tasks": {"task1": 1, "task2": 1}, "partial": True, "init_lambda": 0.6,
                         "router_hidden_layers": 1, "batch_size": 1, "train": True, "lr": 1e-2, "num_steps": 1000,
                         "alpha": 1, "save_interval": 500})
